...
class TNLayer(tf.keras.layers.Layer):  # type:ignore
    def __init__(self) -> None:
        ...
        self.a_var = tf.Variable(
            tf.random.normal(shape=(32, 32, 2), stddev=1.0 / 32.0), name="a", trainable=True
        )
        self.b_var = tf.Variable(
            tf.random.normal(shape=(32, 32, 2), stddev=1.0 / 32.0), name="b", trainable=True
        )
    ...
    def call(self, inputs: Any) -> Any:
        ...
        return tf.nn.relu(tf.reshape(result, (-1, 1024)))  # バッチの数だけの1024ベクトルができる。
